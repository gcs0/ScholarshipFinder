...
logging.basicConfig(
    ...
    handlers=[UTF8FileHandler("csv_import.log"), logging.StreamHandler()],
)
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--overwrite", action="store_true", help="Overwrite existing records"
        )
    ...
    def handle(self, *args, **options):
                ...
                for row_num in range(4, len(rows)):
                    ...
                    try:
                        ...
                        with transaction.atomic():
                            ...
                            if created:
                                stats["created"] += 1
                                safe_name = scholarship_data["scholarship_name"][
                                    :50
                                ].replace("\n", " ")
        ...
        self.stdout.write(
            self.style.SUCCESS(
                f"Import completed: {stats['successful']} successful, {stats['errors']} errors"
            )
        )
    ...
    def clean_old_logs(self):
        ...
        try:
            ...
            if os.path.exists(log_file):
                ...
                if timezone.now() - file_time.replace(tzinfo=timezone.utc) > timedelta(
                    weeks=1
                ):
                    os.remove(log_file)
